...
public class DeckReaderTest {
  ...
  Card cavalry = new Card("Cavalry", 2, 1, oneSided, Player.PLAYER1);
  Card flippedCavalry = new Card("Cavalry", 2, 1, mirrored, Player.PLAYER2);
  ...
  public void testReadDeck() {
    ...
    Assert.assertEquals(readFile(Player.PLAYER1, get(file1.getPath())).get(0), bee);
    // When made into a card, PLAYER2 units (blue) are flipped horizontally upon construction.
    Assert.assertEquals(readFile(Player.PLAYER1, get(file2.getPath())).get(0), cavalry);
    Assert.assertEquals(readFile(Player.PLAYER2, get(file2.getPath())).get(0), flippedCavalry);
    //        //When the 'C' is in the incorrect position, an error is thrown.
    //        assertThrows(IllegalArgumentException.class,
    //                () -> readFile(Player.PLAYER1, Paths.get(file3.getPath())).get(0));
  }
  ...
  public void testIncorrectConstruction() {
    ...
    try {
      ...
      writeToFile("Cavalry 2 1\n" + "XXXXX\n" + "XXXXX\n"
              + "IIXXX\n" + "XXXXX\n" + "XXXXX", file3);
      readFile(Player.PLAYER1, get(file3.getPath()));
  ...
  public void testIncorrectConstruction3() {
    ...
    writeToFile("Cavalry 2 1\n" + "XXXXX\n" + "XXXXX\n" + "IICX\n" + "XXXXX\n" + "XXXXX", file3);
    assertThrows(
        IllegalArgumentException.class, () -> readFile(Player.PLAYER1, get(file3.getPath())));
    writeToFile("Cavalry 2 1\n" + "XXXXX\n" + "XXXXX\n" + "IICXXX\n" + "XXXXX\n" + "XXXXX", file3);
    assertThrows(
        IllegalArgumentException.class, () -> readFile(Player.PLAYER1, get(file3.getPath())));
    assertThrows(IllegalArgumentException.class, () -> readFile(null, get(file2.getPath())));
    assertThrows(IllegalArgumentException.class, () -> readFile(Player.PLAYER1, null));
