...
public class QueensBlood {
  ...
  public QueensBlood(int width, int height, Random r) {
    ...
    for (int i = width - 1; i >= 0; i--) {
      for (int j = height - 1; j >= 0; j--) {
        if (i == width - 1) {
          this.board[i][j] =
              new Pawns(1, Player.PLAYER2); // PLAYER1 and 2 here may need to be swapped later.
        } else if (i == 0) {
          this.board[i][j] = new Pawns(1, Player.PLAYER1);
        } else {
   ...
   */
  public boolean checkPosition(Player player, int cardIdx, int x, int y) {
    try {
      getCell(x, y);
      checkHand(player, cardIdx);
  ...
  private void checkHand(Player player, int cardIdx) {
    try {
      if (player == Player.PLAYER1) {
        this.hand1.get(cardIdx);
      } else {
        this.hand2.get(cardIdx);
      }
    } catch (IndexOutOfBoundsException e) {
